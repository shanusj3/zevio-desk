...
export function useCustomersQuery() {
  return useQuery<Customer[], Error>({
    ...
    queryFn: () => customersApi.fetchAll().then((page) => page.customers),
    staleTime: 1000 * 60 * 2,
    retry: 2,
  });
}
...
export function useInfiniteCustomersQuery(params: FetchCustomersParams = {}) {
  return useInfiniteQuery<PaginatedCustomers, Error>({
    queryKey: queryKeys.customers.infinite(params.search, params.alphabet),
...
export function useCreateCustomerMutation() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: queryKeys.customers.infinite() });
    },
  });
}
...
export function useUpdateCustomerMutation() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (variables: { id: string; data: Partial<Customer> }) => customersApi.update(variables.id, variables.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.customers.all });
      queryClient.invalidateQueries({ queryKey: queryKeys.customers.infinite() });
    },
